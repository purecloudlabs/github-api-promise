...
module.exports = {
	...
	getReadme: function(owner, repo, params) {
		return req.standardRequest(`${config.host}/repos/${owner}/${repo}/readme?${req.assembleQueryParams(params,
			['ref'])}`);
	},
	...
	getContents: function(owner, repo, path, params) {
		return req.standardRequest(`${config.host}/repos/${owner}/${repo}/contents/${path}?${req.assembleQueryParams(params,
			['ref'])}`);
	},
